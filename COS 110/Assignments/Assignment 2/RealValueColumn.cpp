...
RealValueColumn::RealValueColumn(std::string columnName, int columnSize) // Function 1
                : Column(columnName, columnSize)
{
    // Initialise The array member to have columnSize number "Null" elements
    array = new RealElement*[this->columnSize];
...
RealValueColumn::~RealValueColumn() // Function 2
{
    if (array)
    {
...
RealValueColumn* RealValueColumn::clone() const // Function 3
{
    RealValueColumn* clone = new RealValueColumn(columnName, columnSize);
...
RealValueColumn& RealValueColumn::operator+=(std::string data) // Function 4 / 9
{
    if (this->array) // If array exists, append
    {
        // Create a element using "data"
        RealElement* newElement = new RealElement(data);

        // Create a copy of the array with the added size
        int newColumnSize = this->columnSize + 1;
        RealElement** newArray = new RealElement*[newColumnSize];
    ...
    }else
    {
        ...
        this->array = new RealElement*[1];
        this->array[0] = new RealElement(data);
...
RealValueColumn& RealValueColumn::operator-=(std::string data) // Function 5 / 10
{
...
RealValueColumn& RealValueColumn::operator-=(int index) // Function 6 / 11
{
...
int RealValueColumn::operator()(std::string data) const // Function 7 / 12
{
...
RealValueColumn& RealValueColumn::operator()(std::string value, std::string newValue) // Function 8 / 13
{
    if (array)
    {
        ...
        if (index < columnSize)
        {
            ...
            delete this->array[index];
            this->array[index] = new RealElement(newValue);
        }
    }
...
const RealElement* RealValueColumn::operator[](int pos) const // Function 9/ 14
{
...
const RealElement* RealValueColumn::operator[](std::string value) const// Function 10 / 15
{
    // Iterate through array to find the passed in data
    if (array)
    {
...
bool RealValueColumn::operator!() const// Function 11 / 16
{
    bool isValid = false;
